...
import { gdcLearnHtml, gdcYoutubeListHtml } from "./gdc-lessons.js";
import { unitVideosHtml, channelsHtml } from "./videos.js";
import {
  loadProgress,
  saveProgress,
  markLesson,
    ...
    <div class="grid-2">
      ...
      <div class="card">
        ...
        <div class="actions">
          ...
          <button class="secondary" data-go="resources">Textbook, videos and answer keys</button>
        </div>
      </div>
    </div>
...
function gdcView() {
  ...
  return `<div class="main">
    ...
    ${gdcLearnHtml(math, state.gdcTopic || "all")}
  </div>`;
}
...
function bindGdcTopics() {
  app.querySelectorAll("[data-gdc-topic]").forEach((el) => {
    el.addEventListener("click", () => {
      state.gdcTopic = el.getAttribute("data-gdc-topic");
      const host = app.querySelector(".gdc-learn");
      if (!host) return;
      host.outerHTML = gdcLearnHtml(math, state.gdcTopic);
      bindGdcTopics();
    });
  });
}
